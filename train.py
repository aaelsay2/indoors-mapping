...
def optimize_model():
  ...
  if double_dqn:
    next_state_indexes = torch.zeros(BATCH_SIZE, device=device, dtype=torch.long)
    next_state_indexes[non_final_mask] = policy_net(non_final_next_states).max(1)[1].detach()

    next_state_values = torch.zeros(BATCH_SIZE, device=device)
    next_state_values[non_final_mask] = target_net(non_final_next_states).gather(1, next_state_indexes[non_final_mask].unsqueeze(1)).squeeze().detach()
